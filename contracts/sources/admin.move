/// Admin entry points: war creation, tribe registration, and system configuration.
///
/// High-level API for admin-only operations. Calls into config, registry, schedule,
/// snapshots, and systems modules. Requires WarAdminCap authorization.
module lineage_war::admin;

use std::string::String;
use std::option::Option;
use lineage_war::{config, registry, rules, schedule, snapshots, systems};

/// Creates a new lineage war with initial state and returns the admin cap.
...
/// Registers a tribe in an existing war.
public fun register_tribe(
    registry: &mut registry::WarRegistry,
    admin_cap: &registry::WarAdminCap,
    tribe_id: u32,
    display_name: String,
) {
    registry::register_tribe(registry, admin_cap, tribe_id, display_name);
}

/// Schedules a war to end at the specified time.
public fun end_lineage_war(
    registry: &mut registry::WarRegistry,
    admin_cap: &registry::WarAdminCap,
    ended_at_ms: u64,
) {
    registry::end_war(registry, admin_cap, ended_at_ms);
}

/// Publishes the initial war-level configuration defaults.
...
/// Registers a system and publishes its initial configuration.
...
/// Schedules a system config change to take effect at a future time.
public fun schedule_system_rule_change(
    ...
    let change = schedule::schedule_config_change(
        war_id,
        change_id,
        rules::target_kind_system_config(),
        target_system_id,
        config_object_id,
        effective_from_ms,
        created_at_ms,
...
/// Commits a snapshot record for a system state at a specific timestamp.
