/// War and system configuration management: versioned configs, rules, and dynamic fields.
///
/// Manages WarConfigVersion, SystemConfigVersion, and PhaseConfig (all shared/stored).
/// Tracks assembly families, storage types, and rule flags via dynamic fields.
/// Admin-operated via WarAdminCap authorization.
...
/// Publishes a new war-level configuration version.
...
/// Publishes a phase-level configuration with optional tick override and multiplier.
...
/// Publishes a system-level configuration version with enabled status, points, and margins.
...
/// Shares a war config version as a shared object.
public fun share_war_config_version(cfg: WarConfigVersion) {
    transfer::share_object(cfg);
}

/// Shares a phase config as a shared object.
public fun share_phase_config(cfg: PhaseConfig) {
    transfer::share_object(cfg);
}

/// Shares a system config version as a shared object.
public fun share_system_config_version(cfg: SystemConfigVersion) {
    transfer::share_object(cfg);
}

/// Allows a specific assembly family in this system config.
...
/// Allows a specific assembly type ID in this system config.
...
/// Allows a specific storage type ID in this system config.
...
/// Requires a specific item type ID in this system config.
...
/// Returns points per tick.
public fun points_per_tick(cfg: &SystemConfigVersion): u64 {
    cfg.points_per_tick
}

/// Returns take margin for control acquisition.
public fun take_margin(cfg: &SystemConfigVersion): u16 {
    cfg.take_margin
}

/// Returns hold margin for maintaining control.
